...
1. Create a temporary markdown file in the host OS temp directory, not the current workspace. Use a safe temp-file command such as `New-TemporaryFile` on PowerShell or `mktemp -t handoff-XXXXXX.md` on POSIX shells, and read the created file before writing if the command creates an empty file.
...
4. Redact sensitive information, including API keys, passwords, access tokens, and personally identifiable information.
5. Include a "suggested skills" section with any relevant Codex skills for the next session using `$skill-name` syntax.
6. Tell the user the handoff path.
